...
To discuss:
 - Wrapper objects / using primitive types with generics
...
class ListTests {
	...
	void tests() {
		...
		assertEquals( 130, allrunners.totalOf(new DistanceProducer()));  //total distance of all runners
		
		assertEquals( 7, names.totalOf(new StringLengthProducer()));     // total length of all strings
		
		assertEquals(ficbooks, allbooks.filter(new FictionSelector()));
		
		assertEquals(new ConsLoT<Runner>(br, new ConsLoT<Runner>(fs, new ConsLoT<Runner>(jb, mtLoT))),
				allrunners.filter(new Over40Selector()));
		             
		assertEquals(under20, allbooks.filter(new LessThanSelector(20)));
...
//produces the Runner's distances 
class DistanceProducer implements NumberFunction<Runner> {
  public int apply(Runner obj) {
      return obj.getDist();
  }
}

//produces the Runner's distances 
class StringLengthProducer implements NumberFunction<String> {
  public int apply(String obj) {
      return obj.length();
  }
}

class Over40Selector implements IPredicate<Runner> {
  
  //determines if the runner is over 40
  public boolean select(Runner obj) {
      return obj.isOver40();
  }
}
  
class LessThanSelector implements IPredicate<Book> {
  int price;
  
  public LessThanSelector(int price) {
      this.price = price;
  }
  
  //determines if the book's price is less than this price
  public boolean select(Book obj) {
      return obj.lessThan(price);
  }
}

class FictionSelector implements IPredicate<Book> {
  //determines if the book is fiction
  public boolean select(Book obj) {
      return obj.isFiction();
  }
}
